...
{
   printf("i2c init for SCCB.\n");
   i2c1_setup(I2C_PRESCALER, I2C_CTR_EN);
}

uint8_t SCCB1_WriteByte(uint8_t WriteAddress, uint8_t SendByte)
{
   i2c1_send_data(0x42);  //write ID R/Wn = 0
   i2c1_send_command(I2C_START_WRITE);
   i2c1_get_ack();  //can don't care
    
   i2c1_send_data(WriteAddress);
   i2c1_send_command(I2C_WRITE);
   i2c1_get_ack();

   i2c1_send_data(SendByte);
   i2c1_send_command(I2C_WRITE);
   i2c1_get_ack();

   i2c1_send_command(I2C_STOP);
   while(i2c1_busy());
   
   return 1;
}

uint8_t SCCB1_ReadByte(uint8_t ReadAddress)
{
   int ver;
   i2c1_send_data(0x42);  //write ID R/Wn = 0
   i2c1_send_command(I2C_START_WRITE);
   ...
   i2c1_send_data(0x43);  //write ID R/Wn = 1
   i2c1_send_command(I2C_START_WRITE);
   i2c1_get_ack();  //can don't care

   i2c1_send_command(I2C_READ);
   i2c1_get_ack();
   ver = i2c1_get_data();
   i2c1_send_command(I2C_STOP_READ);
   //i2c_get_ack();
   return ver;
}
